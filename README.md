...
- Dopo il punto ultimo del materiale, la curva prosegue fino al primo evento tra il calo resistente `postUltimateMomentDrop` riferito a `Mu` sul ramo discendente (default `0.15`) e la curvatura `maxPostUltimateCurvatureRatio * phiMaterialUltimate` (default `1.20`). Il termine e riportato in `outputs.postUltimateTerminationPoint` e `outputs.terminationReason`; i motivi ordinari sono `post-ultimate-moment-drop` e `post-ultimate-curvature-limit`. `outputs.maximumMomentPoint` resta indipendente. I campi `postPeakMomentDrop` e `postPeakDropPoint` sono mantenuti come alias legacy del criterio di calo.
- Dopo la deformazione ultima la tensione scende a zero per default; il softening lineare e attivato solo con `postUltimateResponse: "linear-softening"` e `postUltimateFractureEnergyDensity`.
